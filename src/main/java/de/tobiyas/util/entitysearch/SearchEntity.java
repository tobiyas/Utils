	...
	public static Entity getEntityInLineOfSight(int maxDistance, LivingEntity toSearchFrom){
		...
		while(blockIt.hasNext()){
			...
			for(Entity entity : nearEntities){
				...
				//Distance to 1 up
				distance = block.getLocation().distanceSquared(entity.getLocation().clone().add(0, 1, 0));
				if(distance < 1){
					return entity;
				}
				
				//Distance to 2 up
				distance = block.getLocation().distanceSquared(entity.getLocation().clone().add(0, 2, 0));
				if(distance < 1){
					return entity;
				}
