...
public class MC_1_7_R4_VollotileCode extends VollotileCode {
	...
	public MC_1_7_R4_VollotileCode() {
		super("v1_7_R4");
	}
	...
	public void playCriticalHitEffect(Player toSendTo, Entity toPlayEffect) {
		try {
			Object mcEntity = ReflectionsHelper
					.getMCEntityFromBukkitEntity(toPlayEffect);
			Object mcPlayer = ReflectionsHelper
					.getMCEntityFromBukkitEntity(toSendTo);

			Method playOutAnnimation = mcPlayer.getClass().getDeclaredMethod(
					"b",
					Class.forName("net.minecraft.server." + CB_RELOCATION
							+ ".Entity"));
	...
	@Override
	public void sendParticleEffect(ParticleEffects effect, Location loc,
			Vector width, float speed, int amount, Player player) {
		if (width == null)
			width = new Vector();

		try {
			PacketPlayOutWorldParticles packet = new PacketPlayOutWorldParticles(
					effect.getPacketArg(), (float) loc.getX(),
					(float) loc.getY(), (float) loc.getZ(), width.getBlockX(),
					width.getBlockY(), width.getBlockZ(), speed, amount);
	...
	public void overwriteAIToDoNothing(LivingEntity entity) {
		try {
			...
			listField.setAccessible(true);
			list = (List<PathfinderGoal>) listField.get(goals);
			list.clear();
