...
public class PlayerDisplayModifier {
    ...
    private static final String PROFILE_URL_ADDITION = "?unsigned=false";
    ...
    private String getProfileJson(String name) throws IOException {
        final URL url = new URL(PROFILE_URL + extractUUID(name).toString().replace("-", "") + PROFILE_URL_ADDITION);
        ...
        return CharStreams.toString(new InputSupplier<InputStreamReader>() {
        	...
        	if(exp.getMessage().toLowerCase().startsWith("Server returned HTTP response code: 429".toLowerCase())){
        		return "";
        	}
